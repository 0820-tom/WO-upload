...
def run_automation(res, status_callback=None):
    ...
    # 서버 환경에서 chromium 자동 설치
    try:
        subprocess.run(["playwright", "install", "chromium"], check=True, capture_output=True)
    except Exception as e:
        logging.warning(f"playwright install 실패 (무시): {e}")
        ...
        try:
            ...
            while retry_count < max_retries and not subject_btn_found:
                try:
                    ...
                    try:
                        page.goto(login['login_url'], timeout=30000)
                        log_status(f"📡 페이지 로드 완료: {page.url}")
                    except Exception as goto_e:
                        log_status(f"❌ 페이지 접속 실패: {goto_e}")
                        raise
